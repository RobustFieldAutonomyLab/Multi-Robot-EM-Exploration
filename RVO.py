...
class RVO_agent:
    ...
    def act(self, observation):
        ...
        for i in range(0, len(dynamic)):
            if np.abs(dynamic[i][0]) < 1e-3 and np.abs(dynamic[i][1]) < 1e-3:
                # padding
                continue

            vB = [dynamic[i][2], dynamic[i][3]]
            pB = [dynamic[i][0], dynamic[i][1]]
        ...
        for i in range(0, len(static)):
            if np.abs(static[i][0]) < 1e-3 and np.abs(static[i][1]) < 1e-3:
                # padding
                continue

            vB = [0.0, 0.0]
            pB = [static[i][0], static[i][1]]
