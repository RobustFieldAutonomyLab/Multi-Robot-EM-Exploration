...
from nav.utils import get_symbol, local_to_world_values, generate_virtual_waypoints

DEBUG_BSP = False


class BeliefSpacePlanning:
    ...
    def prior_gtsam_format(self, measurement, robot_id=None, id0=None):
        return gtsam.PriorFactorPose2(get_symbol(robot_id, id0),
                                      gtsam.Pose2(measurement[0], measurement[1], measurement[2]),
                                      self.prior_noise_model)
    ...
    def waypoints2landmark_observations(self, waypoints, robot_id=None, graph=None, initial_estimate=None):
        ...
        for i, waypoint in enumerate(waypoints):
            ...
            if i == 0:
                ...
                initial_estimate.insert(get_symbol(robot_id, id_initial),
                                        gtsam.Pose2(waypoint[0], waypoint[1], waypoint[2]))
                graph.add(self.prior_gtsam_format(waypoint, robot_id=robot_id, id0=id_initial))
                continue
    ...
    def optimize_virtual_observation_graph(self, graph: gtsam.NonlinearFactorGraph, initial_estimate: gtsam.Values):
        ...
        #     print("initial_estimate: ", initial_estimate, file=file)
        #     print("graph: ", graph, file=file)
        isam.update(graph, initial_estimate)
        result = isam.calculateEstimate()
        marginals = gtsam.Marginals(isam.getFactorsUnsafe(), result)
